...
from OCR.document import load_pages, page_text
...
def ocr_record(record_id: str, record_dir: Path | None = None) -> list[dict]:
    ...
    if mv_config.use_blob_ocr():
        if not blob_store.storage_configured():
            raise SystemExit("OCR_SOURCE=blob but Azure Blob Storage is not configured in .env")
    ...
    if record_dir is None:
        record_dir = mv_config.record_raw_dir(record_id)
    ...
    return load_pages(ocr_read_dir, record_id)
...
def run() -> pd.DataFrame:
    ...
    logger.info("OCR_SOURCE=%s", mv_config.OCR_SOURCE)

    if mv_config.use_blob_ocr():
        record_ids = blob_store.list_ocr_record_ids()
        record_source = f"{blob_store.AZURE_STORAGE_CONTAINER}/{blob_store.AZURE_STORAGE_WRITE_PREFIX}"
    else:
        record_ids = [path.name for path in list_raw_records(mv_config.RAW_Read_Path)]
        record_source = str(mv_config.RAW_Read_Path)
