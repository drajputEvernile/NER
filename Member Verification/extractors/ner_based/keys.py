...
def key_windows(ocr_text: str, group: str, radius: int = WINDOW) -> list[tuple[str, str]]:
    """Return (key, window_text) spans around matching keys."""
    ...
    for key in keys_for(group):
        ...
        for index in range(0, len(folded) - length + 1):
            ...
            if span in seen:
                continue
            seen.add(span)
            windows.append((key, " ".join(tokens[start:stop])))
    return windows
